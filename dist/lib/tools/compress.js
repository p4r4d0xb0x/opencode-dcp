...
export function createCompressTool(ctx) {
    ...
    return tool({
        description: runtimePrompts.compress + (useFlatSchema ? FLAT_FORMAT_OVERLAY : NESTED_FORMAT_OVERLAY),
        ...
        async execute(args, toolCtx) {
            ...
            const missingRequiredBlockIds = validateSummaryPlaceholders(parsedPlaceholders, range.requiredBlockIds, range.startReference, range.endReference, searchContext.summaryByBlockId);
            const injected = injectBlockPlaceholders(compressArgs.content.summary, parsedPlaceholders, searchContext.summaryByBlockId, range.startReference, range.endReference);
            ...
            const summaryWithProtectedTools = await appendProtectedTools(ctx.client, ctx.state, ctx.config.experimental.allowSubAgents, summaryWithUserMessages, range, searchContext, ctx.config.compress.protectedTools, ctx.config.protectedFilePatterns);
            const finalSummaryResult = appendMissingBlockSummaries(summaryWithProtectedTools, missingRequiredBlockIds, searchContext.summaryByBlockId, injected.consumedBlockIds);
            ...
            }, range, anchorMessageId, blockId, storedSummary, finalSummaryResult.consumedBlockIds);
