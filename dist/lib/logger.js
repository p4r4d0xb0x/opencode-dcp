...
export class Logger {
            ...
            if (msg.parts) {
                minimized.parts = msg.parts
                    ...
                    if (part.type === "text") {
                        ...
                        if (part.metadata)
                            textPart.metadata = part.metadata;
                        return textPart;
                    }
                    if (part.type === "reasoning") {
                        const reasoningPart = { type: "reasoning", text: part.text };
                        if (part.metadata)
                            reasoningPart.metadata = part.metadata;
    ...
    async saveContext(sessionId, messages) {
        ...
        try {
            ...
            const minimized = this.minimizeForDebug(messages).filter((msg) => msg.parts && msg.parts.length > 0);
